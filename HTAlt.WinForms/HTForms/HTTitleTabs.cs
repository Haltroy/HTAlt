...
namespace HTAlt.WinForms
{
    ...
    public abstract partial class HTTitleTabs : Form
    {
        ...
        private readonly Uri wikiLink = new Uri("https://haltroy.com/htalt/HTAlt.WinForms/HTTitleTabs");
        ...
        /// <summary>
        /// Event delegate for <see cref="HTTitleTabs.TabDeselecting" /> and <see cref="HTTitleTabs.TabSelecting" /> that allows subscribers to cancel the
        /// event and keep it from proceeding.
        /// </summary>
        /// <param name="sender">Object for which this event was raised.</param>
        /// <param name="e">Data associated with the event.</param>
        public delegate void HTTitleTabCancelEventHandler(object sender, HTTitleTabCancelEventArgs e);

        /// <summary>Event delegate for <see cref="HTTitleTabs.TabSelected" /> and <see cref="HTTitleTabs.TabDeselected" />.</summary>
        /// <param name="sender">Object for which this event was raised.</param>
        /// <param name="e">Data associated with the event.</param>
        public delegate void HTTitleTabEventHandler(object sender, HTTitleTabEventArgs e);
        ...
        protected internal HTTitleTabsOverlay _overlay;
        ...
        /// </summary>
        protected HTTitleTab _previousActiveTab = null;
        ...
        protected HTBaseTabRenderer _tabRenderer;

        /// <summary>List of tabs to display for this window.</summary>
        protected ListWithEvents<HTTitleTab> _tabs = new ListWithEvents<HTTitleTab>();

        /// <summary>Default constructor.</summary>
        protected HTTitleTabs()
        {
        ...
        public bool AeroPeekEnabled
        {
            ...
            set
            {
                ...
                if (!_aeroPeekEnabled)
                {
                    foreach (HTTitleTab tab in Tabs)
                    {
        ...
        /// <summary>List of tabs to display for this window.</summary>
        public ListWithEvents<HTTitleTab> Tabs => _tabs;

        /// <summary>The renderer to use when drawing the tabs.</summary>
        public HTBaseTabRenderer TabRenderer
        {
            get => _tabRenderer;
        ...
        public HTTitleTab SelectedTab
        {
            get => Tabs.FirstOrDefault((HTTitleTab t) => t.Active);
        ...
        {
            get => Tabs.FindIndex((HTTitleTab t) => t.Active);

            set
            {
                HTTitleTab selectedTab = SelectedTab;
                int selectedTabIndex = SelectedTabIndex;

                if (selectedTab != null && selectedTabIndex != value)
                {
                    // Raise the TabDeselecting event
                    HTTitleTabCancelEventArgs e = new HTTitleTabCancelEventArgs
                    {
        ...
        public HTTitleTabsApplicationContext ApplicationContext
        {
            get;
            internal set;
        }
        ...
        {
            base.OnLoad(e);
            _overlay = HTTitleTabsOverlay.GetInstance(this);
        ...
        public event HTTitleTabCancelEventHandler TabDeselecting;

        /// <summary>Event that is raised after a tab has been deselected.</summary>
        public event HTTitleTabEventHandler TabDeselected;

        /// <summary>Event that is raised immediately prior to a tab being selected (<see cref="TabSelected" />).</summary>
        public event HTTitleTabCancelEventHandler TabSelecting;

        /// <summary>Event that is raised after a tab has been selected.</summary>
        public event HTTitleTabEventHandler TabSelected;

        /// <summary>Event that is raised after a tab has been clicked.</summary>
        public event HTTitleTabEventHandler TabClicked;

        /// <summary>
        /// Callback that should be implemented by the inheriting class that will create a new <see cref="HTTitleTab" /> object when the add button is
        /// clicked.
        /// </summary>
        /// <returns>A newly created tab.</returns>
        public abstract HTTitleTab CreateTab();

        /// <summary>Callback for the <see cref="TabClicked" /> event.</summary>
        /// <param name="e">Arguments associated with the event.</param>
        protected internal void OnTabClicked(HTTitleTabEventArgs e)
        {
            if (TabClicked != null)
            {
                TabClicked(this, e);
        ...
        /// <summary>
        /// Callback for the <see cref="TabDeselecting" /> event.  Called when a <see cref="HTTitleTab" /> is in the process of losing focus.  Grabs an image of
        /// the tab's content to be used when Aero Peek is activated.
        /// </summary>
        /// <param name="e">Arguments associated with the event.</param>
        protected void OnTabDeselecting(HTTitleTabCancelEventArgs e)
        {
        ...
        protected void UpdateTabThumbnail(HTTitleTab tab)
        {
        ...
        /// <param name="e">Arguments associated with the event.</param>
        protected void OnTabDeselected(HTTitleTabEventArgs e)
        {
        ...
        /// <param name="e">Arguments associated with the event.</param>
        protected void OnTabSelecting(HTTitleTabCancelEventArgs e)
        {
        ...
        /// <summary>
        /// Callback for the <see cref="TabSelected" /> event.  Called when a <see cref="HTTitleTab" /> gains focus.  Sets the active window in Aero Peek via a
        /// call to <see cref="TabbedThumbnailManager.SetActiveTab(Control)" />.
        /// </summary>
        /// <param name="e">Arguments associated with the event.</param>
        protected void OnTabSelected(HTTitleTabEventArgs e)
        {
        ...
        /// <summary>
        /// Handler method that's called when Aero Peek needs to display a thumbnail for a <see cref="HTTitleTab" />; finds the preview bitmap generated in
        ...
        {
            foreach (
                HTTitleTab rdcWindow in Tabs.Where(rdcWindow => rdcWindow.Content.Handle == e.WindowHandle && _previews.ContainsKey(rdcWindow.Content)))
            {
        ...
        /// <summary>
        /// Callback for the <see cref="Control.ClientSizeChanged" /> event that resizes the <see cref="HTTitleTab.Content" /> form of the currently selected
        ...
        /// <summary>Resizes the <see cref="HTTitleTab.Content" /> form of the <paramref name="tab" /> to match the size of the client area for this window.</summary>
        /// <param name="tab">Tab whose <see cref="HTTitleTab.Content" /> form we should resize; if not specified, we default to
        /// <see cref="SelectedTab" />.</param>
        public void ResizeTabContents(HTTitleTab tab = null)
        {
            if (tab == null)
            {
                tab = SelectedTab;
        ...
        /// <summary>Forwards a message received by <see cref="HTTitleTabsOverlay" /> to the underlying window.</summary>
        ...
        {
            foreach (HTTitleTab tab in Tabs.Where(tab => tab.Content.Handle == e.WindowHandle))
            {
        ...
        private void _tabs_CollectionModified(object sender, ListModificationEventArgs e)
        {
            ...
            if (e.Modification == ListModification.ItemAdded || e.Modification == ListModification.RangeAdded)
            {
                ...
                {
                    HTTitleTab currentTab = Tabs[i + e.StartIndex];

                    currentTab.Content.TextChanged += Content_TextChanged;
                    currentTab.Closing += HTTitleTabs_Closing;
        ...
        protected virtual TabbedThumbnail CreateThumbnailPreview(HTTitleTab tab)
        {
            TabbedThumbnail preview = TaskbarManager.Instance.TabbedThumbnail.GetThumbnailPreview(tab.Content);
        ...
        public virtual void UpdateThumbnailPreviewIcon(HTTitleTab tab, Icon icon = null)
        {
            if (!AeroPeekEnabled)
            {
                return;
        ...
        /// Aero Peek preview.
        /// </summary>
        /// <param name="sender">Object from which this event originated (the <see cref="HTTitleTab.Content" /> object in this case).</param>
        /// <param name="e">Arguments associated with the event.</param>
        ...
        /// <summary>
        /// Event handler that is called when a tab's <see cref="HTTitleTab.Closing" /> event is fired, which removes the tab from <see cref="Tabs" /> and
        /// re-renders <see cref="_overlay" />.
        /// </summary>
        /// <param name="sender">Object from which this event originated (the <see cref="HTTitleTab" /> in this case).</param>
        /// <param name="e">Arguments associated with the event.</param>
        private void HTTitleTabs_Closing(object sender, CancelEventArgs e)
        {
            if (e.Cancel)
            {
                return;
            ...
            HTTitleTab tab = (HTTitleTab)sender;
            CloseTab(tab);
        ...
        /// <summary>
        /// Calls <see cref="HTTitleTabsOverlay.Render(bool)"/> on <see cref="_overlay"/> to force a redrawing of the tabs.
        /// </summary>
        ...
        protected virtual void CloseTab(HTTitleTab closingTab)
        {

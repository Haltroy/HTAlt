...
{
	/// <summary>
	/// Borderless overlay window that is moved with and rendered on top of the non-client area of a  <see cref="HTTitleBarTabs" /> instance that's responsible
	/// for rendering the actual tab content and responding to click events for those tabs.
	/// </summary>
	public class HTTitleBarTabsOverlay : Form
	{
		...
		protected static Dictionary<HTTitleBarTabs, HTTitleBarTabsOverlay> _parents = new Dictionary<HTTitleBarTabs, HTTitleBarTabsOverlay>();

		/// <summary>Tab that has been torn off from this window and is being dragged.</summary>
		protected static HTTitleBarTab _tornTab;
		...
		/// window.
		/// </summary>
		protected Tuple<HTTitleBarTabs, Rectangle>[] _dropAreas = null;
		...
		protected HTTitleBarTabsOverlay(HTTitleBarTabs parentForm)
		{
		...
		public static HTTitleBarTabsOverlay GetInstance(HTTitleBarTabs parentForm)
		{
			if (!_parents.ContainsKey(parentForm))
			{
				_parents.Add(parentForm, new HTTitleBarTabsOverlay(parentForm));
			}
		...
		{
			if (e.Cancel) return;
			HTTitleBarTabs form = (HTTitleBarTabs) sender;
		...
		private void ShowTooltip(HTTitleBarTabs tabsForm, string caption)
		{
		...
		private void ShowTooltipTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
		{
			...
			HTTitleBarTab hoverTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition);

			if (hoverTab != null)
			{
				HTTitleBarTabs hoverTabForm = hoverTab.Parent;
		...
		protected void InterpretMouseEvents()
		{
			foreach (MouseEvent mouseEvent in _mouseEvents.GetConsumingEnumerable())
			{
				...
				if (nCode >= 0 && (int) WM.WM_MOUSEMOVE == (int) wParam)
				{
					...
					if (_tornTab != null && _dropAreas != null)
					{
						...
						// ReSharper restore ForCanBeConvertedToForeach
						{
							...
							{
								HTTitleBarTab tabToCombine = null;
									...
									if (!dragArea.Contains(cursorPosition) && _tornTab == null)
									{
										...
										if (_tornTab != null)
										{
											...
											_dropAreas = (from window in _parentForm.ApplicationContext.OpenWindows.Where(w => w.Tabs.Count > 0)
														  select new Tuple<HTTitleBarTabs, Rectangle>(window, window.TabDropArea)).ToArray();
										}
									}
								}));
					}
				...
				else if (nCode >= 0 && (int) WM.WM_LBUTTONUP == (int) wParam)
				{
					...
					if (_tornTab != null)
					{
						HTTitleBarTab tabToRelease = null;
						...
						if (tabToRelease != null)
						{
							Invoke(
								new Action(
									() =>
									{
										HTTitleBarTabs newWindow = (HTTitleBarTabs) Activator.CreateInstance(_parentForm.GetType());
			...
			switch ((WM) m.Msg)
			{
				...
				case WM.WM_LBUTTONDOWN:
					...
					else
					{
						...
						HTTitleBarTab clickedTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition);
				...
				case WM.WM_NCMBUTTONUP:
					...
					{
						// When the user clicks a mouse button, save the tab that the user was over so we can respond properly when the mouse button is released
						HTTitleBarTab clickedTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition2);
		...
		/// <summary>
		/// Contains information on mouse events captured by <see cref="HTTitleBarTabsOverlay.MouseHookCallback" /> and processed by
		/// <see cref="HTTitleBarTabsOverlay.InterpretMouseEvents" />.
		/// </summary>
